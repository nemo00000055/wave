...
function rollRarity(bonus=0){
  const weights = RARITIES.map(r => {
    let w = r.weight * Math.pow(1.08, bonus);
...
export function createPotion(){
  return { id: uid(), kind:"potion", name:"Healing Potion", healPct:0.35, price:20 };
}
...
export function createEquipment(type, bonus=0){
  ...
  const item = {
    ...
    type,
    rarityKey,
    name: `${rarityKey} ${type[0].toUpperCase()+type.slice(1)}`,
  ...
  if(Math.random() < 0.45){
    ...
    item.setKey = setNames[(Math.random()*setNames.length)|0];
  }
